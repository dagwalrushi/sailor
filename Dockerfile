...
ADD https://downloads.apache.org/tomcat/tomcat-9/v9.0.64/bin/apache-tomcat-9.0.64.tar.gz .
RUN tar -xzvf openjdk-11.0.1_linux-x64_bin.tar.gz
RUN tar -xzvf apache-tomcat-9.0.64.tar.gz
RUN rm -rf apache-tomcat-9.0.64.tar.gz
RUN rm -rf openjdk-11.0.1_linux-x64_bin.tar.gz
COPY opt/jenkins/workspace/sailornew/target/sailor.war $TOMCAT_HOME/webapps
COPY run.sh .
